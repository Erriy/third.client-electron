...
export default {
    ...
    methods: {
        select_seed(item) {
            let seedid = item.meta.id;
            sessionStorage[seedid] = JSON.stringify(item);
        ...
        load_more(page) {
            ...
            .then(async (res)=>{
                ...
                for(let s of res.data.list) {
                    this.seed.list.push(await this.format_seed(s));
                }
    ...
    mounted() {
        this.$common.set_title('节点列表')
        this.$api.menu.update({seed: {list: 'disable'}});
